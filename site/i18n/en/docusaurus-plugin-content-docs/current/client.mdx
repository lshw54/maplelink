...
The 🎮 at the top right of the title bar, or Toolbox → Game client manager. The window has two tabs: **Check & repair** and **Download & install**.
...
<details>
<summary>Does "wrong size" or "wrong content" mean it's broken?</summary>

Not necessarily. The manifest is that version's **base** client, and the game's own updater modifies some files after install — `MapleStory.exe` above all. MapleLink knows about this: it asks the official patch server what the current executable is, and treats a match as fine.
