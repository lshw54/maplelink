  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-2">
        ...
        <span className="w-10 shrink-0 text-right font-mono text-[11px] text-text-dim">
          {pct.toFixed(0)}%
        </span>
      </div>
...
/**
 * The same chrome as the main window: this window is borderless, so the drag
 * region and the buttons have to be drawn here.
 */
function ClientTitlebar({ subtitle }: { subtitle: string }) {
  const { t } = useTranslation();
  ...
  return (
    <div
      ...
      className="flex h-[34px] shrink-0 items-center border-b border-[var(--tb-border)] bg-[var(--tb-bg)]"
    >
      <span className="px-3 text-[12px] font-bold tracking-[1px] text-[var(--text)]">
        {t("client.title")}
      </span>
      <span className="min-w-0 flex-1 truncate text-[11px] text-text-faint">{subtitle}</span>
      <button
